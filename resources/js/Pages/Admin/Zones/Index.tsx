...
export default function ZoneManagement() {
  const { zones, regions } = usePage<PageProps>().props;
  ...
  const handleModalDelete = () => {
    if (deleteZoneId !== null) {
      router.delete(route('admin.zones.destroy', deleteZoneId), {
        preserveScroll: true,
        onSuccess: () => {
  ...
  const handleEditSubmit = () => {
    if (editZone) {
      router.put(route('admin.zones.update', editZone.id), editZone, {
        preserveScroll: true,
        onSuccess: () => {
    ...
    >
      ...
      {showAlert && (
        <div className="mb-6 p-4 bg-blue-50 border-l-4 border-blue-500 text-blue-700">
          <div className="flex justify-between items-center">
            <p>Please verify zone information before making changes.</p>
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="text-sm text-gray-500 dark:text-gray-400">
              Showing {filteredZones.length} of {zones.total} zones
            </span>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              {filteredZones.map((zone: Zone) => (
                <tr key={zone.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
          ...
          <div className="flex items-center justify-between mt-6 border-t border-gray-200 dark:border-gray-700 pt-4">
            ...
            <div className="flex space-x-2">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-md flex items-center ${
                  zones.current_page === zones.last_page
                    ? 'bg-gray-100 dark:bg-gray-700 text-gray-400 cursor-not-allowed'
                    : 'bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'
                }`}
              >
      ...
      <dialog id="delete_modal" className="modal">
        <div className="modal-box border border-red-300 bg-white rounded-2xl shadow-2xl animate__animated animate__fadeInDown">
          ...
          <p className="py-4 text-gray-700">Are you sure you want to delete this zone?</p>
      ...
      <dialog id="view_modal" className="modal">
        <div className="modal-box bg-white rounded-2xl shadow-2xl animate__animated animate__fadeInDown">
          <h3 className="text-xl font-bold text-blue-600 mb-4">📄 Zone Details</h3>
          {viewZone && (
            <div className="mt-2 space-y-3 text-sm text-gray-700">
              <p><span className="font-semibold">ID:</span> {viewZone.id}</p>
              <p><span className="font-semibold">Name:</span> {viewZone.name}</p>
              <p><span className="font-semibold">Code:</span> {viewZone.code}</p>
              <p><span className="font-semibold">Region:</span> {viewZone.region.name}</p>
              <p><span className="font-semibold">Description:</span> {viewZone.description}</p>
              <p><span className="font-semibold">Created:</span> {new Date(viewZone.created_at).toLocaleDateString()}</p>
            </div>
          )}
      ...
      <dialog id="edit_modal" className="modal">
        <div className="modal-box bg-white border border-green-200 rounded-2xl shadow-2xl animate__animated animate__fadeInDown">
          <h3 className="text-xl font-bold text-green-600 mb-4">✏️ Edit Zone</h3>
          {editZone && (
          ...
          <form onSubmit={handleCreateSubmit} className="space-y-6">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setCreateData('region_id', e.target.value)}
                className="mt-1 w-full px-4 py-2 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-green-500 focus:outline-none"
              >
